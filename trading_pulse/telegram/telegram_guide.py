...
def get_telegram_guide(cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    outgoing = [
        ...
        },
        {
            ...
            "parts": [
                ...
                "יש מזומן פנוי: אפשר לקנות ממזומן (כן / קנה / סכום) ו/או להחליף החזקה חלשה "
                "(החלף X Y) כשהציון ברור",
                "מזומן $0: לא מציעים קנייה ב־$0 — רק «החלף FROM TO» כשהציון לטובת החדשה, או דלג",
                "«כן» בלי מזומן לא מאשר קנייה ב־$0 — הבוט מציע במקום פקודת החלף",
                "תשובה: כן / קנה (בסכום המוצע) · סכום (למשל 150 או קנה 150) · החלף X Y · דלג",
                "«החלף FROM TO» כש־TO היא ההצעה הנוכחית = אישור ההצעה "
    ...
    tips = [
        ...
        (
            f"הצעות קנייה בסקירה לפני הפתיחה ({review_t} ישראל): כן / סכום / החלף / דלג — "
            "הבוט ממתין לתשובה לפני שממשיך למניה הבאה. "
            "«החלף FROM TO» כש־TO היא ההצעה = אישור ועוברים הלאה (בלי כן/דלג). "
            "מזומן $0: כן לא מאשר קנייה — מציע החלף כשהציון ברור, אחרת דלג."
        ),
        (
            "החלף לפני הפתיחה: מוכר עכשיו ומאשר קנייה בפתיחה — בלי «שלב 2 — חלוקת הון». "
            "בפתיחת השוק: מה שכבר אושר (גם ב־החלף) נשאר; רק «לא נענו» יורדים."
        ),
        (
                ...
                "items": [
                    {
                        "cmd": "כן",
                        "desc": (
                            "לקנות ממזומן בסכום המומלץ; בלי מזומן — הבוט מציע "
                            "החלף X Y במקום לאשר $0"
                        ),
                    },
                    {"cmd": "150", "desc": "לקנות בסכום מדויק ממזומן (גם $150) — לא הסכום המומלץ"},
                    {
                        "cmd": "החלף LABU HOOD",
                        "desc": (
                            "כש־HOOD היא ההצעה הנוכחית — מוכר LABU עכשיו, מאשר HOOD לפתיחה, "
                            "ועובר להצעה הבאה (בלי כן/דלג); לא נפתח כרטיס חלוקת הון"
                        ),
                    },
