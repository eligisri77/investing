...
def user_guide_full() -> str:
    return "\n".join(
        [
            ...
            "2. בפתיחה — תמונת כניסה; נרות סיניים 2 (עד כמה סטאפים) ממתינות לפריצה ולא נקנות אוטומטית",
            ...
            "(«מימון מסכום שאושר») גם בלי מזומן ובלי החזקות — "
            "<code>כן</code> = פרוסה שווה · או <code>החלף ELF U 190</code>",
            "אין מזומן ויש החזקות? אם יש מניה חלשה יותר בציונים — <code>החלף PBF SOXL</code> · "
            "אחרת <code>מכור ELF 100</code> / <code>מכור 1 $100</code> / <code>מכור 50% ELF</code> / <code>מכור 1</code>",
...
def format_holding_actions(
    ...
    if has_manual:
        lines.append(
            "<i>טיפ: מכירה חלקית — <code>מכור ELF 100</code> / <code>מכור 50% ELF</code> "
            "(גם <code>מכירה ELF 100</code>)</i>"
        )
    return lines
...
def format_portfolio(data: dict[str, Any]) -> str:
    ...
    if holding:
        lines.extend(
            [
                "",
                "<i>דוגמאות: מכור ELF 100 · מכור 1 $100 · מכור 50% ELF · "
                "מכור 1 200$ קנה 2 100$ · תקנה 1 $20</i>",
            ]
        )
