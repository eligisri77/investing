...
**הצעות אחת-אחת (`offer_queue.py`)** — לכל מניה: גרף PNG (כולל שיטת כניסה + בפתיחה/בפריצה) → כרטיס קוביות מדדים («הצעה N/M», כולל קוביית שיטה) → הודעת פעולה קצרה (שיטה + מזומן + `כן`/`קנה`/סכום/`דלג`) — בלי חומת הסבר בטקסט; Method2 = בפריצה בלבד, שאר השיטות = בפתיחה אם אושר; הבוט ממתין לתשובה; תזכורת אחרי ~10 דק׳ בלי תשובה; `החלף FROM TO` כש־TO ההצעה הנוכחית = אישור ועוברים הלאה (לפני פתיחה: מוכר עכשיו · מאשר קנייה בפתיחה — בלי שלב 2 / ח1…ח5); בפתיחת השוק — רק מה שלא נענה יורד (`לא נענו`; `קנה SYMBOL` תמיד עובד בנפרד), מה שכבר אושר (גם ב־החלף) נשאר  
**בוקר (~13:35 UTC)** — כניסה במחיר פתיחה למי ש־`entry_policy=market_open` + הודעה; Method2 נשאר על פריצה  
**מהלך היום** — מעקב שעתי (`intraday_check_*`, ברירת מחדל כל 60 דק׳ בשעות מסחר): אם מזומן פנוי ≥ `intraday_cash_topup_min_usd` (~$20) — סקירה + הצעה בכל סיבוב (קנייה חדשה אם יש מקום, אחרת `תקנה SYMBOL`); בלי מזומן — רק חריגות/החלפות. הצעות מזומן יכולות לחזור כל שעה (לא נחסמות ב־cooldown הארוך של התראות)  
