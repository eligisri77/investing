...
def _intraday_selection_block(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "title": "מעקב במהלך יום המסחר",
        "detail": (
            f"בין {open_t} ל-{close_t} UTC ({freq}): "
            f"מזומן פנוי ≥ ${topup_min:.0f} → בכל בדיקה סקירה + הצעה "
            "(קנייה חדשה אם יש מקום ומניה חזקה; אחרת תקנה SYMBOL להחזקה הכי חזקה). "
            "הצעות פריסת מזומן יכולות לחזור בכל שעה — לא נחסמות ע״י השהיית התראות (~120 דק׳). "
            "בלי מזומן מספיק — רק חריגות/החלפות בולטות. "
            "תמונת PNG (כיתוב «מעקב שעתי») — טבלת HTML עם כותרות עבריות "
            "(מושקע, מחיר, רף יציאה, שווי, מהכניסה, היום, רווח); "
            "אחוזים ודולרים חתומים: -3.5% / -$4.00. "
...
def get_selection_guide(cfg: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    user_flow = [
        {
            ...
            "detail": (
                ...
                "עונים כן / קנה / קנה WDC (אותה הצעה) / קנה WDC 75 / קנה 100 WDC / "
                "סכום / החלף כמו בכרטיס / דלג — כולם מקדמים את התור על ההצעה הנוכחית. "
                "«החלף AMZN TSLL» כש־TSLL היא ההצעה = אישור (מוכר עכשיו · מאשר קנייה בפתיחה) "
                "ועוברים הלאה — בלי כן/דלג ובלי «שלב 2 — חלוקת הון». "
                "אחרי אישור כזה: הצעות הבאות יכולות לקחת חלק מסכום שאושר "
                "(«מימון מסכום שאושר» — כן = פרוסה שווה · או החלף ELF U 190) "
                "גם כשאין מזומן ואין החזקות — לא «אין מניות בתיק». "
                "תיק לא ריק — תמונת קוביות «סקירת תיק» שמתחילה ב«מניות בתיק» "
                "(רשימת הסימבולים) ואז לכל החזקה: הון · PnL% · ציון · המלצה "
                "+ רצועת פעולה קצרה; החלפה לפי ציון מגמה+תיקון (~2+ נקודות) גם בירוק "
                "(אלא אם ממש רווח / קרוב לרצפה) — ואז אותו סבב הצעות אם יש מקום. "
                "בלי תשובה כ-10 דקות — תזכורת עדינה אחת. הפקודות הרגילות (מכור, קנה על מניה אחרת, תיק) עובדות תמיד."
            ),
        },
        {
            "step": "ב",
        ...
        },
        {
            ...
            "detail": (
                f"בשעות המסחר: מזומן פנוי ≥ ${topup_min:.0f} → בכל בדיקה "
                "(ברירת מחדל כל שעה) נשלחת הצעה — קנייה חדשה אם יש מקום ומניה חזקה, "
                "אחרת חיזוק ההחזקה הכי חזקה (תקנה SYMBOL). "
                "הצעות פריסת מזומן יכולות לחזור כל שעה (לא נחסמות ע״י השהיית התראות ~120 דק׳). "
                "בלי מזומן מספיק — רק חריגות/החלפות בולטות."
            ),
        },
    ]
    ...
    return {
        ...
        "pipeline": [
            {
                ...
                "detail": (
                    ...
                    "דירוג: מגמה מעל MA20 + תיקון קצר (לא רדיפה אחרי שיא) · ATR/נפח מתון"
                    + (
            ...
            },
            {
                ...
                "detail": (
                    ...
                    f"בתוכנית היומית היא סורקת עד {cfg.get('method2_scan_cap', 200)} מניות "
                    "(קודם רשימת השבוע, ואז השלמה מהיקום — לא רק ה־60 השבועיות) "
                    f"ומציעה עד {cfg.get('method2_max_offers', 3)} סטאפים מובילים (לא רק אחד). "
                    "טריגר 2-2-2: עם HTF ירוק — נר 2↑ ואז 2↓ (תיקון) — כניסה בפריצה מעל גבוה האמצעי "
                    "(לא שני נרות 2 באותו כיוון). "
                    "שורט: מראה — 2↓ ואז 2↑, כניסה בפריצה מתחת לנמוך האמצעי (רווח כשהמחיר יורד). "
            ...
            },
            {
                ...
                "detail": (
                    ...
                    "(שיטת כניסה · ציון ותנודתיות · מומנטום ומחיר = מגמה+תיקון · נפח · חדשות ומקורות · בדיקה לאחור) "
                    "+ מזומן וקנייה · החלפה מומלצת אם רלוונטי · איך לבצע — בלי הודעת פעולה נפרדת. "
                    "«החלפה מומלצת»: גם מפת כסף עכשיו ($ לכל החזקה + מזומן) וכמה מעבירים מהחלשה — לא רק ציונים; "
                    "במזומן $0 עם החזקות — אותה מפה גם ב«מזומן וקנייה». "
                    "ב«איך לבצע»: כן / קנה ממזומן, או פקודות עם סימבולים אמיתיים מהתיק "
                    "(למשל החלף AMZN TSLL · מכור AMZN · מכור ELF 100) — לא המילה SYMBOL. "
                    "או בדשבורד (#/plan) — עונים כן / קנה / קנה WDC (אותה הצעה) / "
                    "קנה WDC 75 / קנה 100 WDC / סכום מדויק / החלף כמו בכרטיס / דלג — "
                    "כולם מקדמים את התור על ההצעה הנוכחית. "
                    "כל הצעה מושווית להחזקות: יש מזומן — קנייה ממזומן ו/או החלפת החזקה חלשה; "
                    "מזומן $0 ויש החזקות — מימון בהחלף/מכור עם סימבולים אמיתיים מהתיק "
                    "(לא SYMBOL) כשהציון ברור (~2+), או דלג. "
                    "אחרי «החלף» שאישר מניה לפתיחה: גם בתיק ריק ומזומן $0 — "
                    "כרטיס «מימון מסכום שאושר» (כן = פרוסה שווה מהסכום שאושר; "
                    "או החלף ELF U 190) — לא «אין מניות בתיק». "
                    "«כן» בלי מזומן ובלי סכום שאושר מציע החלף עם סימבולים מהתיק במקום לאשר $0. "
        ...
        "strategy": {
            ...
            "scoring": (
                [
                    "מגמה: מעל MA20 (מתחת — ציון נמוך)",
                    "תיקון קצר: ירידה יומית / נסיגה משיא 20 יום (−8%…−2%) / זיגזג בטווח",
                    "קנס על רדיפה: קרוב מדי לשיא או עלייה חזקה מדי ב-5 ימים",
                    "ATR% ונפח — בונוס מתון (לא עיקר הציון)",
                    "בונוס נושא קטן (+1) לקוונטים / Data center (רשימה פנימית)",
                ]
                if speculative
                else [
                    "מגמה: מעל MA20 (מתחת — ציון נמוך)",
                    "תיקון קצר: ירידה יומית / נסיגה משיא 20 יום (−8%…−2%) / זיגזג בטווח",
                    "קנס על רדיפה: קרוב מדי לשיא או עלייה חזקה מדי ב-5 ימים",
                    "נפח מול ממוצע — בונוס מתון",
                    "בונוס נושא קטן (+1) לקוונטים / Data center (רשימה פנימית)",
                ]
            ),
            "must_pass": (
                "הציון מעדיף מגמה חיובית + כניסה בתיקון — לא רדיפה אחרי שיאים. "
                "אותו ציון משמש גם להחלפות בתיק ולמעקב שעתי. "
                "כל בחירה עדיין כפופה למגבלות הון וסיכון."
            ),
            "stops": (
        ...
        "enrichment": [
            ...
            },
            {
                ...
                "title": "בונוס נושא",
                "detail": (
                    "מניות קוונטים (למשל IONQ, RGTI) ותשתית Data center "
                    "(למשל SMCI, VRT, EQIX) מקבלות בונוס ציון קטן (+1) — "
                    "רשימה פנימית, בלי לגבור על מגמה+תיקון."
                ),
            },
            {
                "icon": "📰",
            ...
            },
            {
                ...
                "title": "דירוג סופי",
                "detail": (
                    "אחרי חדשות — מיון מחדש לפי ציון סופי "
                    "(מגמה+תיקון, לא רדיפה אחרי שיא). רק הראשונות נכנסות לתוכנית."
                ),
            },
        ],
        "limits": [
